	...
	tests := []struct {
		...
		{"sub path", args{
			json: `[{"a": [{"b": 1, "c": 3}, {"b": 2, "c": 1}], "c": "dd"},
                    ...
                    {"a": [{"b": 3, "c": 2}, {"b": 3, "c": 3}]}]`,
			stmt: `find $val where [$val /a [$b1 /b neq(2)][$b2 /c neq(2)]]`},
			[]byte(`[{"val":[{"b":1,"c":3},{"b":2,"c":1}]},{"val":[{"b":3,"c":2},{"b":3,"c":3}]}]`),
		},
		{"root", args{
			json: `[{"a": [{"b": 1, "c": 2}, {"b": 2, "c": 1}], "c": "dd", "k": 2.5},
                    {"a": [{"b": 2, "c": 2}, {"b": 3, "c": 2}], "d": "a", "k": 1.5},
                    {"a": [{"b": 3, "c": 2}, {"b": 3, "c": 3}]}]`,
			stmt: `find $val where [$val /c] [/k neq(3.5)]`},
			[]byte(`[{"val":"dd"},{"val":null}]`),
		},
	}
