...
func TestGreater(t *testing.T) {
	...
	tests := []struct {
		...
		{"float", args{
			json: `[{"a": [{"b": 1, "c": 3}, {"b": 2, "c": 1}], "c": "dd", "k": 2.5},
                    {"a": [{"b": 2, "c": 2}, {"b": 3, "c": 2}], "d": "a", "k": 3.5},
                    {"a": [{"b": 3, "c": 2}, {"b": 3, "c": 3}]}]`,
			stmt: `find $val where [$val /a] [/k gt(3.2)][/k ge(3.0)]`},
			[]byte(`[{"val":[{"b":2,"c":2},{"b":3,"c":2}]}]`),
		},
		{"int", args{
			json: `[{"a": [{"b": 1, "c": 2}, {"b": 2, "c": 1}], "c": "dd", "k": 2},
                    {"a": [{"b": 2, "c": 2}, {"b": 3, "c": 2}], "d": "a", "k": 1},
                    {"a": [{"b": 3, "c": 2}, {"b": 3, "c": 3}]}]`,
			stmt: `find $val where [$val /c] [/k ge(2)] [/k gt(1)]`},
			[]byte(`[{"val":"dd"}]`),
		},
	}
